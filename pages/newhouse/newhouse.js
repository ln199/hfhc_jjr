...
Page({
  ...
  getOptions() {
    ...
    wx.request({
      ...
      success(res) {
        // 价格筛选过滤  ++ 格式初始化
        var oneArr = [];
        oneArr = res.data.data.xfprice.filter(function (element, index, all) {
        ...
        var twoArr = [];
        twoArr = res.data.data.xfprice.filter(function (element, index, all) {
        ...
        if (res.data.code == 0) {
          var contents = res.data.data.ysfsort;
          var obj = {
            id: 0,
            comment: "默认排序"
          }
  ...
  getHouseList() {
    ...
    if (this.data.price_id == undefined) {
      this.data.price_id = ''
    }
      ...
      success(res) {
        if (res.data.code == 0) {
          ...
          for (let i = 0; i < result.length; i++) {
            if (result[i].tese.length != 0) {
              result[i].tese = result[i].tese.slice(0, 3);
            }
          }
  ...
  search(e) {   
    wx.navigateTo({
      url: '../housesearch/housesearch?path=newhouse'
    })
  ...
  hrefDetail(e) {
    ...
    if (this.data.into_type == 1) { // 房源列表-》详情-》报备进入的
    ...
    } else if(this.data.into_type == 2){ // 买客客源里面进入的（新房报备）
      let pages = getCurrentPages(); //获取当前页面js里面的pages里的所有信息。      
      let prevPage = pages[ pages.length - 2 ];        
      //prevPage 是获取上一个页面的js里面的pages的所有信息。 -2 是上一个页面，-3是上上个页面以此类推。      
      prevPage.setData({  // 将我们想要传递的参数在这里直接setData。上个页面就会执行这里的操作。
  ...
  selectpr: function (e) {
    this.setData({
      select5: e.target.dataset.price,
      price_id: e.currentTarget.dataset.price_id
    });
  },
