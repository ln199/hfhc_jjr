// pages/housecheckin2/housecheckin2.js
const app = getApp();
Page({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      ...
      fangyuan: fangyuan,
      xiangqing: xiangqing,
      indexl: Number(fangyuan.chaoxiang) - 1,
      indext: Number(fangyuan.zhuangxiu) - 1,
      indexj: Number(fangyuan.xianzhuang) - 1,
      indexly: Number(fangyuan.laiyuan) - 1
    })    
  },
  ...
  getOptions() {
    ...
    wx.request({
      ...
      success(res) {
        ...
        // 处理默认返显
        let pt = _this.data.xiangqing.peitao;
        let bq = _this.data.xiangqing.biaoqian;
        let select_id = [], select_ids = []
        if (pt) {
          for (let i = 0;i < pt.length;i += 1) {
            for (let j = 0;j < list.length;j += 1) {
              if (pt[i].id == list[j].id) {
                list[j].states = 1
                select_id.push(list[j].lxid)
                break;
              }
            }
          }                 
        ...
        if (bq) {
          for (let i = 0;i < bq.length;i += 1) {
            for (let j = 0;j < lists.length;j += 1) {
              if (bq[i].id == lists[j].id) {
                lists[j].states = 1
                select_ids.push(lists[j].lxid)
                break;
              }
            }
          }                   
  ...
  nextStep(e) {
    ...
    if (v.building == "") {
      ...
      wx.navigateTo({
        url: '../edithousecheckins_mzf/edithousecheckins_mzf?id=' + this.data.id,
      })
    }
  },
  /**
