...
Page({
  ...
  getDetail() {
    ...
    wx.request({
      url: _this.data.basicURL + 'chuzufang/content',
      method: "GET",
      data: data,
      header: {
  ...
  hrefSecond(e) {
    wx.navigateTo({
      url: '../rentxq/rentxq?id=' + e.currentTarget.dataset.id,
    })
  },
  // 跳转到新房
  hrefNew(e) {
  ...
  startCooperation() {
    ...
    wx.request({
      ...
      success(res) {
        wx.navigateTo({
          url: '../cooperate/cooperate?id=' + res.data.data.id,
        })
      }
    })
  },
  ...
  onShareAppMessage: function () {
    ...
    return {
      title: title,
      imageUrl: imgUrl,
      path: '/pages/rentxq/rentxq?id='+this.data.id + '&sharename=' + sharename + '&sharephone=' + sharephone
    }  
  }
})