...
Page({
  ...
  //佣金更多
  commission: function () {
    let yongjin = JSON.stringify(this.data.ysfang.yongjin);
    wx.navigateTo({
      url: "../commission/commission?list="+yongjin
    })
  },
  ...
  swiperTab: function (e) {
    if (this.data.current_index >= e.detail.current) {
      e.detail.current = this.data.current_index
    }
    var that = this;
    that.setData({
  ...
  // 周边新房pages/newhouse/newhouse
  intoDetails(e) {
    wx.navigateTo({
      url: './newhousexq?id=' + e.currentTarget.dataset.id,
    })
  },
  // 新房列表
  houseList() {
    ...
    setTimeout(function () {
      ...
      wx.getSystemInfo({
        success: function (res) {
          ...
          if (currentTabs == 0) {
            that.setData({
              scrollHeight: commissionHeight + ruleHeight + boothHeight + housetypeHeight + buildingHeight + 102
            })
          } else if (currentTabs == 1) {
            that.setData({
              scrollHeight: billHeight + highlightsHeight + clauseHeight + 94
            })
          } else if (currentTabs == 2) {
            that.setData({
              scrollHeight: groupdHeight + groupeHeight + 90
            })
          } else if (currentTabs == 3) {
            that.setData({
              scrollHeight: dynamicHeight + 82
            })
          }
        }
      })
  ...
  chat(e) {
    ...
    let site = this.data.ysfang.zhuchang[e.currentTarget.dataset.idx];
    ...
    wx.request({
      ...
      success(res) {
        if (res.data.code == 0) {
          ...
          if(res.data.data.photo.length > 0) {
            current_index = 0;
          }          
          _this.setData({
            ...
            poster: res.data.data.ysfang.poster,
  ...
  // 加载默认图片
  errDetails(e) {
    var tjysf = this.data.tjysf;
    var default_img = e.currentTarget.dataset.img;
    var index = e.currentTarget.dataset.index;
    tjysf[index].img = default_img;
    this.setData({
      tjysf
    })
  },
  ...
  onShareAppMessage: function () {
    ...
    let imgUrl = '', photo = this.data.photo
    if (photo && photo.length > 0) {
      imgUrl = photo[0].img_path
    }
    let title = this.data.ysfang.xmbiaoti + ' ' + this.data.ysfang.junjia + ' ' + this.data.ysfang.lpdizhi
    return {
      title: title,      
      imageUrl: imgUrl,
      path: '/pages/newhousexq/newhousexq?id=' + this.data.id + '&sharename=' + sharename + '&sharephone=' + sharephone
    }
  }
})