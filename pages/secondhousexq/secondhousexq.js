// pages/secondhousexq/secondhousexq.js
...
Page({
  ...
  viewCompletion() {
    wx.navigateTo({
      url: "../cooperate/cooperate?id=" + this.data.completion
    })
  },
  // 详情数据
  getDetail() {
    ...
    wx.request({
      ...
      success(res) {
        if (res.data.code == 0) {
          ...
          if(res.data.data.xiangqing.img.length > 0) {
            current_index = 0;
          } 
          _this.setData({
            ...
            "movies[0].url": res.data.data.xiangqing.vr_url,
            "movies[1].url": res.data.data.xiangqing.shipin_url,
            show_vr: res.data.data.xiangqing.vr_url,
            show_video: res.data.data.xiangqing.shipin_url,
      ...
      success(res) {
        if (res.data.status == 1) {
          if (res.data.data.length == 0) {
            ...
            var userList = JSON.stringify(res.data.data)
            wx.navigateTo({
              url: '../buylist/buylist?userList=' + userList,
            })
          }
        }
      }
    ...
    wx.request({
      ...
      success(res) {
        if (res.data.code == 0) {
          ...
          wx.navigateTo({
            url: '../cooperate/cooperate?id=' + res.data.data.id + "&into_type=1",
          })
        } else {
          wx.showToast({
            title: res.data.msg,
  ...
  onShareAppMessage: function () {
    ...
    return {
      title: title,      
      imageUrl: imgUrl,
      path: '/pages/secondhousexq/secondhousexq?id='+this.data.id + '&sharename=' + sharename + '&sharephone=' + sharephone
    }    
  }
})