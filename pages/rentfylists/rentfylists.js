// pages/rentfylists/rentfylists.js
...
Page({
  ...
  data: {
    ...
    contents: ['默认排序', '单价由低到高', '单价由高到低', '开盘时间顺序', '开盘时间倒序'], //排序列表内容
    show: false,
  ...
  getHouseList() {
    ...
    var data = {
      ...
      // jiage: this.data.minPrice + "," + this.data.maxPrice,
      // huxing: this.data.hxIndex,
      // sort: this.data.pxId
    }
    let gongsiid = wx.getStorageSync('gongsiid')
    if(gongsiid != 0) {
      data.gongsiid = gongsiid
    }
    this.data.reqTask = wx.request({
      url: _this.data.basicURL + 'my/fangyuan',
      method: "POST",
      data: data,
      header: {
  ...
  hrefDetail(e) {
    ...
    wx.navigateTo({
      url: '../houseresourcesxqs/houseresourcesxqs?id=' + id + '&user_id=' + this.data.user_id,
    })
  },
  ...
  onShow: function () {
    ...
    this.getUserInfo(this.getHouseList)
    // this.getHouseList();
    this.getOptions();
  },
  ...
  getOptions() {
    ...
    wx.request({
      ...
      success(res) {
        // 价格筛选过滤  ++ 格式初始化
        var oneArr = [];
        oneArr = res.data.data.zfprice.filter(function (element, index, all) {
        ...
        var twoArr = [];
        twoArr = res.data.data.zfprice.filter(function (element, index, all) {
        ...
        if (res.data.code == 0) {
          var contents = res.data.data.zfsort;
          var obj = {
            id: 0,
            comment: "默认排序"
          }
          ...
          _this.setData({
            ...
            "priceleft[0].item": twoArr,
            "priceleft[1].item": oneArr,
            contents: contents
          })
        }
      }
  ...
  // 价格右侧栏选择的内容
  selectpr: function (e) {
    if (this.data.select4 == 0) {
      this.data.minPrice = e.currentTarget.dataset.min;
      this.data.maxPrice = e.currentTarget.dataset.max;
    } else if (this.data.select4 == 1) {
